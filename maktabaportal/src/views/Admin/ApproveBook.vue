<template>
    ...
    <div class="card" align="left">
      ...
      <div class="card-body">
        <form @submit.prevent="saveData">
          ...
          <div class="row">
            <div class="col-sm-8">
              <input type="submit" value="Approve" class="btn btn-md btn-primary float-end">
            </div>
          </div>
        </form>
      </div>
  ...
  const saveData = async () => {
   try {
    ...
    const response = await axios.post(`http://127.0.0.1:8000/api/approvebook/${loanId}`, formData, {
      headers: {
