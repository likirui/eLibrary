<template>
           ...
           <input type="text" v-model="searchQuery" placeholder="Search..." class="form-control mb-3" />
             <table class="table table-bordered">
              ...
              <tbody v-if="filteredBooks.length >0">
                ...
                <td>  
                    <a class="btn btn-success me-2" @click.prevent="returnBook(book.id)">Return Book</a>
             </td>  
                 </tr>
              </tbody>
              <tbody v-else>
   ...
   try {
     await axios.post(`http://127.0.0.1:8000/api/returnbook/${bookId}`, {
        added_by: userId
     }, {
       headers: {
         ...
         getBooks () {
           const token = localStorage.getItem('token');
           console.log(token);
           axios.get('http://127.0.0.1:8000/api/approvedbooks', {
             headers: {
               'Authorization': `Bearer ${token}`
             }
           }).then(res => {
